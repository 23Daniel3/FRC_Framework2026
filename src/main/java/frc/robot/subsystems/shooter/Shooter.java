...
public class Shooter extends StateSubsystem<ShooterRequest, ShooterState, ShooterIOInputsAutoLogged, ShooterIO> {
  ...
  public Shooter(ShooterIO io) {
    ...
    fsm.addGlobalTransition(ShooterState.IDLE, 
        () -> isRequest(ShooterRequest.STOP) && notInState(ShooterState.IDLE));

    fsm.addGlobalTransition(ShooterState.FLYWHEEL_RAMPING, 
        () -> isRequest(ShooterRequest.SHOOT) && notInState(ShooterState.KICKER_RAMPING) && notInState(ShooterState.SHOOTING));

    fsm.addGlobalTransition(ShooterState.REVERSING, 
        () -> isRequest(ShooterRequest.REVERSE) && notInState(ShooterState.REVERSING));
  }

  private boolean isFlywheelReadyToKick() {
    return SetpointTracker.atSetpoint(velocity.in(RPM), ShooterConstants.START_KICKER_TOLERANCE, inputs.leaderInputs.velocity.in(RPM));
  }
