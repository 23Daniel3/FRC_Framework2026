...
public class SuperStructure extends SubsystemBase {
  ...
  public SuperStructure(
    ...
    this.hubCalculator =
        new ShotOnTheMoveCalculator(
            ...
            flywheelMap, aimScalar, rpmScalar, shooterEfficiency,
            shotConfig);
  ...
  public boolean isAtSetpointAngle() {
    ...
    double tolerance   = isInAllianceZone()
  ...
  private void configureGeneralFSM() {
    ...
    generalFsm
        ...
        .transitionTo(RobotState.SHOOTING,
            () -> shooter.readyToShoot() && isAtSetpointAngle());
    ...
    generalFsm
        ...
        .transitionTo(RobotState.COLLECT_SHOOTING,
            () -> shooter.readyToShoot() && isAtSetpointAngle());
    ...
    generalFsm
        ...
        .transitionTo(RobotState.CLOSED,
            () -> intake.atGoal() && conveyor.atGoal() && shooter.atGoal());
    ...
    generalFsm.addGlobalTransition(RobotState.GOING_SHOOT,
    ...
    generalFsm.addGlobalTransition(RobotState.GOING_COLLECT,
        () -> robotRequest == RobotRequest.COLLECT
            && generalFsm.getCurrentState() != RobotState.COLLECTING);

    generalFsm.addGlobalTransition(RobotState.GOING_COLLECT_SHOOT,
    ...
    generalFsm.addGlobalTransition(RobotState.CLOSING,
        () -> robotRequest == RobotRequest.CLOSE
            && generalFsm.getCurrentState() != RobotState.CLOSED);

    generalFsm.addGlobalTransition(RobotState.IDLEING,
        () -> robotRequest == RobotRequest.IDLE
            && generalFsm.getCurrentState() != RobotState.IDLE);
  }
  ...
  private Translation2d resolveFeedTarget(Pose2d robotPose) {
    ...
    Pose2d left  = alliance == Alliance.Blue ? Poses.SHOOT_INTAKING_LEFT_BLUE  : Poses.SHOOT_INTAKING_LEFT_RED;
    Pose2d right = alliance == Alliance.Blue ? Poses.SHOOT_INTAKING_RIGHT_BLUE : Poses.SHOOT_INTAKING_RIGHT_RED;
  ...
  private static ShotOnTheMoveCalculator.Config buildShotConfig(double rpmSmootherAlpha) {
    double avgDiameter =
        (SuperStructureConstants.DIAMETER_WHEEL_UP_METERS
            + SuperStructureConstants.DIAMETER_WHEEL_DOWN_METERS) / 2.0;
